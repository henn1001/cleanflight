...
#if defined(GPS) && defined(GPS_PROTO_I2C)

#define GPS_I2C_POLL_RATE_HZ    5  // Poll I2C GPS at this rate
...
{
   // We don´t need to Initialize i2c speed.
    return false;
}
...
static bool gpsConfigure(void)
{
    ...
    case 0: // Config
        ...
        if (gpsState.hwVersion < 70000) {
            ...
            if (gpsState.autoConfigPosition < sizeof(ubloxInit6)) {
                i2cGPSModuleWrite(ubloxInit6[gpsState.autoConfigPosition]);
            ...
            if (gpsState.autoConfigPosition < sizeof(ubloxInit)) {
                i2cGPSModuleWrite(ubloxInit[gpsState.autoConfigPosition]);
                gpsState.autoConfigPosition++;
            }
            else {
                gpsState.autoConfigPosition = 0;
    ...
    case 1: // SBAS
        if (gpsState.autoConfigPosition < UBLOX_SBAS_MESSAGE_LENGTH) {
            i2cGPSModuleWrite(ubloxSbas[gpsState.gpsConfig->sbasMode].message[gpsState.autoConfigPosition]);
            gpsState.autoConfigPosition++;
        } else {
            gpsState.autoConfigPosition = 0;
            gpsState.autoConfigStep++;
...
{
    return i2cGPSModuleDetect();
}

bool gpsPollI2C(void)
{
    ...
    uint8_t newChar[256];
    i2cGPSModuleRead((uint8_t *) newChar);

    for (uint8_t i = 0; i < sizeof(newChar); i++)
        gpsNewFrameUBLOX(newChar[i]);
    
    return true;
}

bool gpsHandleI2C(void)
{
    // Process state
