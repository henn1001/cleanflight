...
static void imuCalculateAccelerationAndVelocity(uint32_t deltaT)
{
    ...
    imuUpdateAccelerationAndVelocity(X, applyDeadband(lrintf(accel_ned.V.X), accDeadband), deltaT);
    imuUpdateAccelerationAndVelocity(Y, applyDeadband(lrintf(accel_ned.V.Y), accDeadband), deltaT);
    imuUpdateAccelerationAndVelocity(Z, applyDeadband(lrintf(accel_ned.V.Z), accDeadband), deltaT);
}
